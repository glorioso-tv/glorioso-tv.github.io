...
class BuildMenu:
    ...
    def get_listing(self):
        ...
        if response:
            ...
            directory.add_file('Versao do Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
            directory.add_dir('Menu Salvar Dados', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_separator()
            directory.add_file('URL do arquivo txt invalida', icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        ...
        directory.add_file('Versao do Kodi: {0}'.format(CONFIG.KODIV), icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        directory.add_dir('Menu Salvar Dados', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
        directory.add_separator()
        ...
        if len(match) >= 1:
            if CONFIG.SEPARATE == 'true':
                ...
                if count19 > 0:
                    ...
                    directory.add_file('[B]{0} Builds Matrix ({1})[/B]'.format(state, count19), {'mode': 'togglesetting',
            ...
            if adultcount > 0:
                directory.add_file('Atualmente ha apenas builds adultas', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME3)
                directory.add_file('Ative Mostrar Adultos em Configuracoes do Addon > Diversos', icon=CONFIG.ICONBUILDS,
                                   themeit=CONFIG.THEME3)
            else:
                directory.add_file('Atualmente nao ha builds disponiveis de {0}'.format(CONFIG.ADDONTITLE),
                                   icon=CONFIG.ICONBUILDS, themeit=CONFIG.THEME3)
        else:
            directory.add_file('Arquivo de texto de builds nao esta formatado corretamente.', icon=CONFIG.ICONBUILDS,
                               themeit=CONFIG.THEME3)
    ...
    def view_build(self, name):
        ...
        if not check.check_build(name, 'version'):
            ...
            directory.add_file('{0} nao foi encontrada na lista de builds.'.format(name), themeit=CONFIG.THEME3)
            return
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            if updatecheck:
                build = '{0} [COLOR red][ATUAL v{1}][/COLOR]'.format(build, CONFIG.BUILDVERSION)
                
            directory.add_file(build, description=description, fanart=fanart, icon=icon, themeit=CONFIG.THEME4)
            directory.add_separator()
            directory.add_dir('Menu Salvar Dados', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_file('Informacoes da Build', {'mode': 'buildinfo', 'name': name}, description=description, fanart=fanart,
                               icon=icon, themeit=CONFIG.THEME3)
                               
            if previewcheck:
                directory.add_file('Ver Previa em Video', {'mode': 'buildpreview', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME3)
            
            if versioncheck:
                directory.add_file(
                    '[I]Build criada para Kodi v{0} (instalada: v{1})[/I]'.format(str(kodi), str(CONFIG.KODIV)),
    ...
    def build_info(self, name):
        ...
        if response:
            if check.check_build(name, 'url'):
                ...
                msg = "[COLOR {0}]Nome da Build:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, name)
                msg += "[COLOR {0}]Versao da Build:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, version)
                if themes:
                    msg += "[COLOR {0}]Tema(s) da Build:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, ', '.join(themes))
                msg += "[COLOR {0}]Versao do Kodi:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, kodi)
                msg += "[COLOR {0}]Conteudo Adulto:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, adult)
                msg += "[COLOR {0}]Descricao:[/COLOR] [COLOR {1}]{2}[/COLOR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, description)

                if extend:
                    msg += "[COLOR {0}]Ultima Atualizacao:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, created)
                    msg += "[COLOR {0}]Tamanho Extraido:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(extracted))))
                    msg += "[COLOR {0}]Tamanho do Zip:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, tools.convert_size(int(float(zipsize))))
                    ...
                    msg += "[COLOR {0}]Imagens:[/COLOR] [COLOR {1}]{2}[/COLOR][CR][CR]".format(CONFIG.COLOR2, CONFIG.COLOR1, picture)
                ...
                window.show_text_box("Visualizando Informacoes da Build: {0}".format(name), msg)
            else:
                logging.log("Nome de Build invalido!")
        else:
            logging.log("Arquivo de texto de builds nao esta funcionando: {0}".format(CONFIG.BUILDFILE))
    ...
    def build_video(self, name):
        ...
        if response:
            ...
            if tools.open_url(videofile, check=True):
                ...
                logging.log("[{0}]Nao foi possivel encontrar URL para previa em video".format(name))
        else:
            logging.log("Arquivo de texto de builds nao esta funcionando: {0}".format(CONFIG.BUILDFILE))
    ...
    def create_install_menu(self, name):
        ...
        menu_items.append((CONFIG.THEME3.format('Instalacao Limpa'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=fresh)'.format(CONFIG.ADDON_ID, buildname)))
        menu_items.append((CONFIG.THEME3.format('Instalacao Normal'), 'RunPlugin(plugin://{0}/?mode=install&name={1}&url=normal)'.format(CONFIG.ADDON_ID, buildname)))
        ...
        menu_items.append((CONFIG.THEME3.format('Informacoes da Build'), 'RunPlugin(plugin://{0}/?mode=buildinfo&name={1})'.format(CONFIG.ADDON_ID, buildname)))
