...
def show_apk_warning(apk):
    ...
    xbmc.executebuiltin('Skin.SetString(apkinstaller, Agora que {0} foi baixado[CR]Clique em instalar na proxima janela!)'.format(apk))
    ...
    class FirstRun(xbmcgui.WindowXMLDialog):
        ...
        def onInit(self):
            ...
            self.controllist = [self.trakt, self.debrid, self.login,
                                    self.sources, self.profiles, self.playercore, self.guisettings,
            ...
            self.controlsettings = ['keeptrakt', 'keepdebrid', 'keeplogin',
                                    'keepsources', 'keepprofiles', 'keepplayercore', 'keepguisettings',
...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msg = "Atualmente nenhuma build instalada de {0}.\n\nSelecione 'Menu de Builds' para instalar uma build da comunidade ou 'Ignorar' para nao ver esta mensagem novamente.\n\nObrigado por escolher {1}.".format(CONFIG.ADDONTITLE, CONFIG.ADDONTITLE)
        ...
        def do_build_menu(self):
            logging.log("[Current Build Check] [Usuario selecionou: Abrir Menu de Builds] [Proxima verificacao: {0}]".format(CONFIG.BUILDCHECK),
                        level=xbmc.LOGINFO)
        ...
        def do_ignore(self):
            logging.log("[Current Build Check] [Usuario selecionou: Ignorar Menu de Builds] [Proxima verificacao: {0}]".format(CONFIG.BUILDCHECK),
                        level=xbmc.LOGINFO)
            CONFIG.set_setting('nextbuildcheck', tools.get_date(days=CONFIG.UPDATECHECK, formatted=True))
            CONFIG.set_setting('installed', 'ignored')
...
def show_update_window(name='Janela de Teste', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    class UpdateWindow(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fanart = kwargs['fanart']
            self.msgupdate = "Atualizacao disponivel para a build instalada:\n[COLOR {0}]{1}[/COLOR]\n\nVersao Atual: v[COLOR {2}]{3}[/COLOR]\nUltima Versao: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Recomendado: instalacao limpa[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
        ...
        def do_fresh_install(self):
            logging.log("[Check Updates] [Versao Instalada: {0}] [Versao Atual: {1}] [Usuario selecionou: Instalacao Limpa]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
        ...
        def do_normal_install(self):
            logging.log("[Check Updates] [Versao Instalada: {0}] [Versao Atual: {1}] [Usuario selecionou: Instalacao Normal]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
            logging.log("[Check Updates] [Next Check: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
        ...
        def do_ignore(self):
            logging.log("[Check Updates] [Versao Instalada: {0}] [Versao Atual: {1}] [Usuario selecionou: Ignorar por {2} dias]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST, CONFIG.UPDATECHECK))
            logging.log("[Check Updates] [Next Check: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
            self.close()
    ...
    # update.doModal()
    # del update
    msgcurrent = 'Executando a versao mais recente da build instalada: '
    msgupdate = 'Atualizacao disponivel para a build instalada: '
    build_name = '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name)
    current_version = 'Versao Atual: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, current)
    latest_version = 'Ultima Versao: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
...
def show_notification(msg, test=False):
    class Notification(xbmcgui.WindowXMLDialog):
        ...
        def do_remind(self):
            ...
            logging.log('[Notifications] Notificacao {0} lembrar depois'.format(CONFIG.get_setting('noteid')))
            self.close()

        def do_dismiss(self):
            if not test:
                CONFIG.set_setting('notedismiss', 'true')
            logging.log('[Notifications] Notificacao {0} dispensada'.format(CONFIG.get_setting('noteid')))
            self.close()
...
def show_log_viewer(window_title="Visualizando Arquivo de Log", window_msg=None, log_file=None, ext_buttons=False):
    class LogViewer(xbmcgui.WindowXMLDialog):
        ...
        def onClick(self, controlId):
            if controlId == self.closebutton:
                ...
                if not newmsg:
                    self.setProperty('message.title', "Erro ao Exibir Arquivo de Log")
                    self.setProperty('message.logmsg', "Arquivo nao existe ou nao pode ser lido.")
                else:
                    self.logmsg = newmsg
