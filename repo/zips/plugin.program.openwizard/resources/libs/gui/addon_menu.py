...
class AddonMenu:
    ...
    def show_menu(self, url=None):
        ...
        if url_response:
            ...
            TEMPADDONFILE = None
            logging.log("[Addon Menu] Nenhuma lista de addons adicionada.")
        ...
        if TEMPADDONFILE:
            ...
            try:
                ...
                addons_json = None
                logging.log("[Addon Menu] ERROR: Invalid Format for {0}.".format(TEMPADDONFILE))
            ...
            if addons_json:
                ...
                if addons and len(addons) > 0:
                    for addon in addons:
                        ...
                        if not addonname:
                            logging.log('[Addon Menu] Missing tag \'name\'', level=xbmc.LOGDEBUG)
                            continue

                        if not addonurl:
                            logging.log('[Addon Menu] Missing tag \'url\'', level=xbmc.LOGDEBUG)
                            continue
                        else:
                            if '.zip' in addonurl:
                                pass
                                ...
                                broken = False
                                if not repository:
                                    logging.log('[Addon Menu] Missing tag \'repository\'', level=xbmc.LOGDEBUG)
                                    broken = True
                                if not repositoryxml:
                                    logging.log('[Addon Menu] Missing tag \'repositoryxml\'',
                                                level=xbmc.LOGDEBUG)
                                    broken = True
                                if not repositoryurl:
                                    logging.log('[Addon Menu] Missing tag \'repositoryurl\'',
                                                level=xbmc.LOGDEBUG)
                                    broken = True
                                if broken:
                                    continue
                    ...
                    if not addons:
                        directory.add_file('Arquivo de texto nao formatado corretamente!', themeit=CONFIG.THEME3)
