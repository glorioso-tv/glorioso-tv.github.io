...
class Config:
    ...
    def _resolve_kodi_home_path(self, special_home):
        """
        Resolve Kodi home across common platform paths, including Flatpak.
        Preference order:
        1) Kodi's own special://home
        2) Existing known OS-specific Kodi paths with userdata folder
        """
        candidates = [special_home]
        user_home = os.path.expanduser('~')

        # Linux / Flatpak / common Unix layouts
        candidates.extend([
            os.path.join(user_home, '.kodi'),
            os.path.join(user_home, '.var', 'app', 'tv.kodi.Kodi', '.kodi'),
            '/storage/.kodi',      # LibreELEC/CoreELEC
            '/home/osmc/.kodi',    # OSMC
            '/home/xbmc/.kodi'
        ])

        # macOS
        candidates.append(os.path.join(user_home, 'Library', 'Application Support', 'Kodi'))
        ...
        if localappdata:
            candidates.append(os.path.join(localappdata, 'Kodi'))

        # Android
        ext_storage = os.environ.get('EXTERNAL_STORAGE')
        if ext_storage:
            candidates.append(os.path.join(ext_storage, 'Android', 'data', 'org.xbmc.kodi', 'files', '.kodi'))
        candidates.extend([
        ...
        for path in candidates:
            ...
            userdata = os.path.join(norm, 'userdata')
            if os.path.isdir(userdata):
                return norm

        return os.path.abspath(special_home)
    ...
    def init_paths(self):
        ...
        self.XMLS = ['sources.xml', 'favourites.xml', 'profiles.xml',
                     'playercorefactory.xml', 'guisettings.xml']
        ...
        self.HOME = self._resolve_kodi_home_path(xbmcvfs.translatePath('special://home/'))
        ...
        # Keep userdata aligned with resolved Kodi home when available.
        resolved_userdata = os.path.join(self.HOME, 'userdata')
        if os.path.isdir(resolved_userdata):
            self.USERDATA = resolved_userdata
