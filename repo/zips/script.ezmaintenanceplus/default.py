...
# ######################### CATEGORIES ################################
def CATEGORIES():
    CreateDir('[COLOR red][B]REINICIAR E LIMPAR TUDO[/B][/COLOR]', 'url', 'fresh_start', ADDON_ICON, ADDON_FANART, '')
...
def MAINTENANCE():
    ...
    CreateDir('Limpar cache','url','clear_cache',ADDON_ICON,ADDON_FANART,'')
    CreateDir('Limpar pacotes','url','clear_packages',ADDON_ICON,ADDON_FANART,'')
    CreateDir('Limpar miniaturas','url','clear_thumbs',ADDON_ICON,ADDON_FANART,'')
...
if action   == None: CATEGORIES()
    ...
    if s_type == 0:
        modes = ['Backup completo', 'Configurações do Kodi']
    ...
    elif s_type == 2: wiz.installLocalBuild()
