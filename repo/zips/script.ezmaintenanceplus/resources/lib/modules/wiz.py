    ...
    if os.path.exists(BACKUPDATA):
        if not backupdir == '':
            ...
            if name != "-":
                ...
                exclude_dirs = ['']
                canceled = CreateZip(BACKUPDATA, backup_zip, 'Criando backup', 'Copiando arquivos', exclude_dirs, exclude_database)
                if canceled:
                    os.unlink(backup_zip)
                    dialog.ok(AddonTitle,'Backup cancelado.')
                else:
                    dialog.ok(AddonTitle,'Backup concluído.')
        else:
           dialog.ok(AddonTitle,'Nenhuma pasta de backup configurada.')
...
def restoreFolder():
    ...
    if zipFolder == '' or zipFolder == None:
        control.infoDialog('Configure primeiro a pasta dos backups nas configurações.')
...
def installLocalBuild():
    """Select a downloaded ZIP and install it directly into Kodi."""
    zip_file = dialog.browse(
        1,
        'Selecione o arquivo ZIP da build',
        'files',
        '.zip',
        False,
        False,
    ...
    zip_file = translatePath(zip_file)
    if not os.path.isfile(zip_file):
        dialog.ok(AddonTitle, 'O arquivo ZIP selecionado não foi encontrado.')
        return

    yes_dialog = dialog.yesno(
        AddonTitle,
        'A build será instalada sobre a configuração atual. Continuar?',
        yeslabel='Instalar',
        nolabel='Cancelar',
    )
    if not yes_dialog:
    ...
    dp.create(AddonTitle, 'Instalando a build...', 'Aguarde.')
    canceled = ExtractZip(zip_file, control.HOME, dp)
    dp.close()
    if canceled:
        dialog.ok(AddonTitle, 'Instalação cancelada.')
        return

    dialog.ok(AddonTitle, 'Build instalada com sucesso. O Kodi será reiniciado.')
    xbmc.executebuiltin('LoadProfile(Master user)')

def restore(zipFile):
    yesDialog = dialog.yesno(AddonTitle, 'Isso substituirá as configurações atuais. Deseja continuar?', yeslabel='Sim', nolabel='Não')
    if yesDialog:
        try:
            dp = xbmcgui.DialogProgress()
            dp.create("Restaurando backup", "Em andamento..." + '\n' + "Aguarde.")
            dp.update(0, "" + '\n' + "Extraindo o ZIP. Aguarde.")
            canceled = ExtractZip(zipFile, control.HOME, dp)
            if canceled:
                dialog.ok(AddonTitle,'Restauração cancelada.')
            else:
                dialog.ok(AddonTitle,'Restauração concluída.')
    ...
    for dirpath, dirnames, filenames in os.walk(folder):
        ...
        try:
            ...
            for file in filenames:
                ...
                if PY2:
                    dp.update(int(progress),"Criando backup", 'ARQUIVOS: ' + str(count) + '/' + str(N_ITEM)  + '   [COLOR lime]' + str(file) + '[/COLOR]', 'Aguarde.')
                else:
                    dp.update(int(progress),"Criando backup" + '\n' + 'ARQUIVOS: ' + str(count) + '/' + str(N_ITEM)  + '   [COLOR lime]' + str(file) + '[/COLOR]' + '\n' + 'Aguarde.')
