...
def build_request(url, data=None, headers=None, bump='0', secure=False):
    ...
    if secure and schemed_url.startswith('http://'):
        schemed_url = 'https://' + schemed_url[7:]
...
class SpeedtesterFilePutter(threading.Thread):
    """Send one pre-built upload payload like script.speedtester."""

    def __init__(self, url, start, size, opener=None, shutdown_event=None,
                 secure=False):
        threading.Thread.__init__(self)
        self.url = url
        chars = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'
...
class Speedtest(object):
    ...
    def get_config(self):
        ...
        threads = {
            'upload': max(4, int(upload['threads'])),
    ...
    def get_best_server(self, servers=None):
        ...
        for server in servers:
            ...
            # Three latency requests are made for each server.
    ...
    def upload(self, callback=do_nothing, pre_allocate=True):
        ...
        sizes = []
        for size in (int(.25 * 1000 * 1000), int(.5 * 1000 * 1000)):
            for _ in range(0, 25):
                sizes.append(size)

        request_count = len(sizes)

        def producer(q, sizes, request_count):
            for i, size in enumerate(sizes[:request_count]):
                thread = SpeedtesterFilePutter(
                    self.best['url'], start, size, opener=self._opener,
                    shutdown_event=self._shutdown_event, secure=self._secure
                )
                thread.start()
                q.put(thread, True)
                callback(i, request_count, start=True)
...
def main():
    try:
        official_result = run_official_speedtest()
        if official_result:
            show_official_result(official_result)
            return
...
def find_official_speedtest():
    """Find an Ookla CLI installed on the current device."""
    for name in ('speedtest', 'speedtest.exe'):
...
def run_official_speedtest():
    """Run Ookla locally; no user data is bundled with the addon."""
    executable = find_official_speedtest()
    if not executable:
        return None
...
def show_official_result(result):
    """Display the local Ookla result in Kodi."""
    ping = result.get('ping', {}).get('latency', 0)
    download = result.get('download', {}).get('bandwidth', 0) * 8 / 1000000
    upload = result.get('upload', {}).get('bandwidth', 0) * 8 / 1000000
    xbmcgui.Dialog().ok(
        'Teste de velocidade',
        'Ping: %.0f ms\nDownload: %.2f Mbps\nUpload: %.2f Mbps' %
