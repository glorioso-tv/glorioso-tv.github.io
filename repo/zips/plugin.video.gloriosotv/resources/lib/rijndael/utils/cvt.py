...
def _hex_encode(raw_string):
    if isinstance(raw_string, str):
        raw_string = raw_string.encode('latin-1')
    hex_bytes = binascii.hexlify(raw_string)
    if sys.version_info[0] == 2:
        return hex_bytes
...
class Counter(str):
    ...
    def __init__(self, initial_ctr):
        ...
        self.c = int(_hex_encode(initial_ctr), 16)

    def __call__(self):
        # This might be slow, but it works as a demonstration
        ctr = _hex_decode("%032x" % (self.c,))
        self.c += 1
        return ctr
