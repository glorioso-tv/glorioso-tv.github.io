...
def supports_infotag():
    return kversion >= 19


def set_video_info(listitem, title='', plot='', genre='', mediatype='video'):
    if supports_infotag():
        info = listitem.getVideoInfoTag()
        if title is not None:
            info.setTitle(str(title))
        if plot:
            info.setPlot(str(plot))
        if genre:
            try:
...
def is_xbox_platform():
    try:
        ...
        pass
    try:
        build = xbmc.getInfoLabel('System.BuildVersion').lower()
        if 'xbox' in build or 'uwp' in build:
            return True
    except:
        pass
    return False
...
def addMenuItem(params={}, destiny='', folder=True):
    ...
    info, infotag = set_video_info(li, title=name, plot=description, mediatype=mediatype)
    if year:
        if infotag:
