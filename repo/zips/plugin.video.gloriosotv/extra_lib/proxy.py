...
class ProxyHandler:
    def __init__(self):
        ...
        # Configura retentativas automáticas em caso de queda pontual
        retries = Retry(
            total=5,                  # Tenta até 5 vezes antes de desistir
            backoff_factor=0.3,       # Aguarda pequenos intervalos entre as tentativas
            status_forcelist=[500, 502, 503, 504] # Reconecta se o servidor oscilar
        )
        
        # Aumentamos o pool de 25 para 50 para suportar múltiplos segmentos HLS/TS
        adapter = HTTPAdapter(
            pool_connections=50, 
            pool_maxsize=50, 
            max_retries=retries
