...
def dns_resolver_iptv(url,headers):
    ...
    if tem_ip:
        ...
        try:
            r = requests.get('https://dns.adguard.com/resolve',headers={"Accept": "application/dns-json"}, params=params).json()
...
class XtreamCodes:
    ...
    def base_url(self,url):
        ...
        try:
            if not PARAMS:
                for i in range(7):
                    info = dns_resolver_iptv(url,header_)
                    url = info.get('url', '')
    ...
    def check_url_params(self,url):
        ...
        if URL_BASE_PARAMS:
            ...
            if '.m3u8' in url and CHECK_URL_PARAMS:
                try:
                    info = dns_resolver_iptv(url,HEADERS_BASE)
                    url = info.get('url', '')
                    header_ = info.get('headers', {})                      
                    DNSOverride()
                    r = requests.get(url,headers=header_,timeout=3,verify=False)
    ...
    def send_ts(self,self_server,url):
        ...
        if '.ts' in url or '/hl' in url and not '.ts' in url:
            ...
            for i in range(MAX_RETRY):
                ...
                try:
                    header_ = self.update_headers()
                    info = dns_resolver_iptv(ts,header_)
                    ts = info.get('url', '')
                    header_ = info.get('headers', {})                      
                    DNSOverride()
