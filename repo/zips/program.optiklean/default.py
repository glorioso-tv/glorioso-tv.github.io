...
def show_menu():
    ...
    icons = {
        ...
        "adjust_advanced_settings": f"{media_path}settings.png",
    ...
    menu_items = [
        ...
        (addon.getLocalizedString(31290), "adjust_advanced_settings"),
...
def clear_addon_leftovers(auto_mode=False):
    ...
    addon_data_dir = xbmcvfs.translatePath("special://profile/addon_data/")
...
def optimize_databases(auto_mode=False):
    ...
    if backup_path_setting and backup_path_setting.strip():
        backup_path = backup_restore.safe_path_join(backup_path_setting, 'db_backups')
    else:
        backup_path = backup_restore.safe_path_join(xbmcvfs.translatePath(addon.getAddonInfo('profile')), 'db_backups')
    ...
    try:
        if not xbmcvfs.exists(backup_path) and not xbmcvfs.mkdirs(backup_path):
            raise OSError(f"Unable to create backup directory: {backup_path}")
...
class AllInOnePanelWindow(BaseWindow):
    ...
    def refresh_data(self):
        ...
        try:
            ...
            sizes = self.calculate_sizes()  # Forza ricalcolo ignorando cache
...
def adjust_advanced_settings():
    """Aplica os ajustes de rede recomendados ao advancedsettings.xml do Kodi."""
    advanced_settings_path = xbmcvfs.translatePath("special://profile/advancedsettings.xml")
    network_settings = {
    ...
    try:
        if xbmcvfs.exists(advanced_settings_path):
        ...
        for setting_name, setting_value in network_settings.items():
            setting = network.find(setting_name)
            if setting is None:
                setting = ET.SubElement(network, setting_name)
            setting.text = setting_value

        ET.indent(tree, space="    ")
        content = ET.tostring(root, encoding="utf-8", xml_declaration=True) + b"\n"
        file = xbmcvfs.File(advanced_settings_path, "w")
        file.write(content)
        file.close()

        xbmcgui.Dialog().notification(
            "OptiKlean",
            addon.getLocalizedString(31291),
            logo_path,
            5000
        )
        xbmc.log("OptiKlean: advancedsettings.xml atualizado", xbmc.LOGINFO)
    except Exception as error:
        xbmc.log(f"OptiKlean: Erro ao atualizar advancedsettings.xml: {error}", xbmc.LOGERROR)
        xbmcgui.Dialog().notification(
            addon.getLocalizedString(31218),
            addon.getLocalizedString(31292).format(error=error),
            logo_path,
            5000,
        )
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'autorun':
        ...
        if action == "clear_cache_and_temp":
            ...
            view_logs()
        elif action == "adjust_advanced_settings":
            adjust_advanced_settings()
